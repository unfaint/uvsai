...
def uvsai_id(img_id):
    ...
    vardict['image_answer_u'] = trans[lng]['right_answer'] if int(answers[img_id]) == int(image_gt) else trans[lng][
        'wrong_answer']
    vardict['image_answer_ai'] = trans[lng]['right_answer'] if int(image_pred) == int(image_gt) else trans[lng][
        'wrong_answer']
    ...
    TP = sum(
        [int(int(r.lrange(email + ':answers', 0, -1)[i]) == int(int(r.lrange(email + ':image_gt', 0, -1)[i])) == 1) for
         i in range(len(file_list))])
    FP = sum(
        [int((int(r.lrange(email + ':answers', 0, -1)[i]) == 1) & (int(r.lrange(email + ':image_gt', 0, -1)[i]) == 0))
         for i in range(len(file_list))])
    FN = sum(
        [int((int(r.lrange(email + ':answers', 0, -1)[i]) == 0) & (int(r.lrange(email + ':image_gt', 0, -1)[i]) == 1))
         for i in range(len(file_list))])
    ...
    TP = sum([int(
        (int(r.lrange(email + ':answers', 0, -1)[i]) < 3) & int(r.lrange(email + ':image_pred', 0, -1)[i]) == int(
            int(r.lrange(email + ':image_gt', 0, -1)[i])) == 1) for i in range(len(file_list))])
    FP = sum([int(
        (int(r.lrange(email + ':answers', 0, -1)[i]) < 3) & (int(r.lrange(email + ':image_pred', 0, -1)[i]) == 1) & (
                    int(r.lrange(email + ':image_gt', 0, -1)[i]) == 0)) for i in range(len(file_list))])
    FN = sum([int(
        (int(r.lrange(email + ':answers', 0, -1)[i]) < 3) & (int(r.lrange(email + ':image_pred', 0, -1)[i]) == 0) & (
                    int(r.lrange(email + ':image_gt', 0, -1)[i]) == 1)) for i in range(len(file_list))])
    precision = float(TP / (TP + FP)) if (TP + FP) > 0 else .0
    recall = float(TP / (TP + FN)) if (TP + FN) > 0 else .0
    f1 = float(2 * (precision * recall) / (precision + recall)) if (precision + recall) > 0 else .0
    ...
    for key in result.keys():
        ...
        result[key]['acc'] = float(
            int(val[key + '_ca']) / int(val[key + '_atotal']) if int(val[key + '_atotal']) > 0 else 0) * 100
